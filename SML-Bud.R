...
hist(df$rent_full, breaks = 100, xlab = "Rent in CHF", col = "red", main = "")
...
table(df$cheminee)
prop.table(table(df$cheminee))

table(df$newly_built)
prop.table(table(df$newly_built))

table(df$parking_indoor)
prop.table(table(df$parking_indoor))
...
table(stack(df, c("balcony",  "basement", "bath", "bright" ,"cabletv", "cheminee", "dishwasher",
                 "dryer", "elevator", "furnished", "heating_air", "heating_earth",
                  ...
                  "shower","sunny", "terrace", "toilets", "topstorage","veranda","water")), useNA = "always", drop = FALSE)

table(stack(df, c("cheminee", "newly_built")))
...
model.m <- lm(rent_full~area+ Micro_rating+dist_to_5G+
              ...
              newly_built+parking_indoor+rooms+msregion, df.train)
...
## Regression Output, stargazr

stargazer(model.m, model.s, model.f, title="Results", align=TRUE)
...
p.m <- model.m %>% predict(x.test, np=10000)
